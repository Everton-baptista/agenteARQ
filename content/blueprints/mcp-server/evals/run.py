...
fixed the lie; this fixes the absence.
    ...
    python evals/run.py --agent tool-gateway            # needs ANTHROPIC_API_KEY
    python evals/run.py --agent tool-gateway --dry-run   # no model calls; checks the harness
...
def main() -> int:
    ...
    parser.add_argument("--agent", default="tool-gateway")
    ...
    principal = Principal(tenant_id="eval", subject="eval-runner", role="account_manager")
    datasets, results = [], []
