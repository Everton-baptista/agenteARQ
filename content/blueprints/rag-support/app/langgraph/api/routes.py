...
from .schemas import AskRequest, AskResponse, ApprovalDecision, Citation, Health
...
async def health() -> Health:
    ...
    # Which credential has to be present is a consequence of model.provider, so it is read from
    # the manifest rather than assumed. A manifest that will not load leaves it empty, and an
    # empty provider resolves no credential — which is the right answer: a replica that cannot
    # read its own contract is not ready, whatever key happens to be in its environment.
...
def _why_degraded(
    ...
    if not has_credential:
        # The provider is named, because "the credential does not resolve" sends an operator to
        # check the key they think is in use, which on a project that switched provider is the
        # wrong one.
        reasons.append(
...
async def ask(
    ...
    with telemetry.span(
        "agent.run",
        **{"tenant": principal.tenant_id, "agent.id": "support-triage"},
    ):
    ...
    return AskResponse(
        ...
        answer=outcome.text,
        citations=[Citation(passage_id=c) for c in outcome.citations],
        tool_calls=outcome.tool_calls,
        cost_usd=round(outcome.cost_usd, 6),
    )
