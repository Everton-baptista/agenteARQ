...
fixed the lie; this fixes the absence.
    ...
    python evals/run.py --agent support-triage            # needs ANTHROPIC_API_KEY
    python evals/run.py --agent support-triage --dry-run   # no model calls; checks the harness
...
from app.agent.manifest import AGENT_DIR, load_manifest  # noqa: E402
from app.agent.principal import Principal  # noqa: E402
from app.agent.retrieval import retrieve  # noqa: E402
...
def run_case(case: Case, principal: Principal, dry_run: bool) -> CaseResult:
    ...
    if dry_run:
        ...
        text = f"[{passages[0]['id']}] simulated" if passages else "I cannot answer that."
    else:
    ...
    ok, _ = output_guardrail(text, bool(cited))
    ...
    return CaseResult(
        ...
        grounded=bool(cited) and (not case.grounded_in or bool(set(cited) & set(case.grounded_in))),
...
def main() -> int:
    ...
    parser.add_argument("--agent", default="support-triage")
