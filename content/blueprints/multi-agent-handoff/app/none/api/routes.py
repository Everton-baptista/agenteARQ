...
from .schemas import AskRequest, AskResponse, ApprovalDecision, BudgetReport, Health
...
async def ask(
    ...
    with telemetry.span(
        "agent.run",
        **{"tenant": principal.tenant_id, "agent.id": "request-orchestrator"},
    ):
    ...
    if outcome.status == "awaiting_approval":
        ...
        return AskResponse(
            ...
            budget=BudgetReport(**outcome.budget) if outcome.budget else None,
        )
