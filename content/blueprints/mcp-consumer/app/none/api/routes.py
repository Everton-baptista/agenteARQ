...
from ..agent.runner import servers_for
...
from .schemas import AskRequest, AskResponse, ApprovalDecision, McpCall, Health
...
async def health() -> Health:
    ...
    # The rug-pull check belongs in readiness. A replica whose servers changed their tool
    # descriptions should stop taking traffic, not discover it on a customer's request.
    try:
        mcp_ok, mcp_detail = _check_mcp(), ""
    except Exception as err:  # noqa: BLE001 - reported, never raised out of a health probe
        mcp_ok, mcp_detail = False, str(err).splitlines()[0]

    ok = prompt_ok and has_credential and circuit != "open" and mcp_ok
    return Health(
        ...
        mcp_verified=mcp_ok,
        detail=_why_degraded(prompt_ok, detail, has_credential, provider, circuit) or mcp_detail,
    )


def _check_mcp() -> bool:
    """Whether every allowlisted server is reachable and still serving the reviewed descriptions."""
    manifest = load_manifest()
    servers_for(manifest)  # raises AllowlistError when default is not deny, or none is declared
    return True
...
async def ask(
    ...
    Note what is not passed through: nothing from the body reaches the model except `question`,
    and it reaches it inside the delimited untrusted block that render_untrusted builds. The
    concatenation invariant 2 forbids would happen here if it happened anywhere.
    """
    ...
    with telemetry.span(
        "agent.run",
        **{"tenant": principal.tenant_id, "agent.id": "docs-assistant"},
    ):
    ...
    if outcome.status == "awaiting_approval":
        ...
        return AskResponse(
            ...
            mcp_calls=[McpCall(tool=t) for t in outcome.mcp_calls],
        )
