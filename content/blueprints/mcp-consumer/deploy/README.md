...
| more than one replica means Redis | the approval store defaults to in-memory, so an approval raised by replica A is a 404 at replica B and the run is lost. See `app/infra/store.py` |

One more, specific to this blueprint: the MCP servers in `agentarch/project/mcp/allowlist.yaml`
must be reachable from where this runs. The allowlist is default-deny with pinned versions, and
`app/infra/mcp.py` refuses a server whose tool descriptions changed since review — so a network
path that works locally and not in production fails closed, at startup, which is what you want.
What you do not want is discovering the egress rule was never opened.
...
gcloud run deploy mcp-consumer \
  --source . \
  --region "$REGION" \
...
- egress: the pods need to reach the allowlisted MCP servers and the model provider, and nothing
  else. A NetworkPolicy that allows exactly those two destinations is the runtime form of the
  allowlist

```bash
kubectl create secret generic mcp-consumer --from-literal=ANTHROPIC_API_KEY="$YOUR_KEY"
...
| `agent.guardrail.decisions{point="input",decision="block"}` | MCP tool results are untrusted content; a rise here can be a poisoned server response, not user misbehaviour |
| MCP server failures, by server | an allowlisted server that starts failing changes what the agent can do; a `RugPull` refusal means its tool descriptions changed since review and it stays refused until a human re-reviews |
