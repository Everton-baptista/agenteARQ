---
lang: pt-BR
source: content/standards/en/09-security.md
source_sha256: "96595a2e057c9bb05050b46f6eafb9f402f9b950fb52b87a3e180f0250f9ee2e"
...
Propósito: o modelo adversarial, e quais controls respondem a cada parte dele.
Versão: 0.1 · Status: draft · Escopo: all agents that read input they did not author.
...
**Intenção.** Raise the cost of the direct attack and catch the unsophisticated case.
**Severidade** `major` · **Fail mode** `fail_closed`
...
**Intenção.** Keep instruction and data structurally separate.
...
**Intenção.** Bound what a fooled model can reach.
**Severidade** `blocker` · **Fail mode** `fail_closed`
...
**Intenção.** Close the outbound channel.
**Severidade** `blocker` · **Fail mode** `fail_closed`
...
**Intenção.** A repository never holds a secret value.
**Severidade** `blocker`
...
**Intenção.** Code the model influenced runs somewhere bounded.
**Severidade** `blocker` for `write`, `irreversible` and `money` effects
...
Revisado em 2026-07-28. Mappings, never the source text — the mapping is the part that stays
