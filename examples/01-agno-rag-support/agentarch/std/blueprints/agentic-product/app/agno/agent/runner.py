"""The agent loop, bounded by the manifest — and stopped by it when a human is required.
...
from .memory import Memory, render_for_prompt
...
def run(
    ...
    # The declared autonomy level, read from the manifest and passed into the action check. This is
    # the value that makes "irreversible at L2 with approval: none" a refusal rather than a policy
    # nobody enforces.
    ...
    if resume:
        ...
        # The operator's request, delimited. It is instruction-shaped and still untrusted: an
        # operator can be phished, and a request that arrives through a ticketing integration was
        # written by whoever opened the ticket.
        turn = f"<operator_request>\n{question}\n</operator_request>"

        # What this tenant remembered from earlier turns, in its own delimited block — invariant 2
        # applied where it is easiest to forget. Memory feels like ours in a way a retrieved page
        # does not, but a fact written on turn 3 and recalled on turn 9 is a delayed injection,
        # and putting it in the system prompt is the delivery it was waiting for.
        if memory is not None:
            recalled = render_for_prompt(memory.recall(principal, conversation_id))
            if recalled:
                turn = f"{recalled}\n\n{turn}"
    ...
    for step in range(max_steps):
        ...
        if not tool_uses:
            ...
            return Outcome(status="answered", text=text, tool_calls=calls, cost_usd=spent)
            ...
            try:
                with telemetry.span(
                    ...
                    **{
                    ...
                    out = dispatch(use.name, use.input, spec, principal, autonomy, approved)
                telemetry.record_guardrail(
                    "action",
