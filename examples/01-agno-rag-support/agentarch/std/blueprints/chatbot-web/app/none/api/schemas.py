...
class AskRequest(BaseModel):
    question: str = Field(
        min_length=1,
        max_length=4000,
        description="The visitor's chat message. Untrusted: treated as data, never as instruction.",
    )
...
class Citation(BaseModel):
    passage_id: str = Field(description="Identifier of the retrieved passage the claim rests on.")


class AskResponse(BaseModel):
    status: Literal["answered", "escalated", "awaiting_approval"] = Field(
        description=(
            "answered: grounded, with at least one citation. "
            "escalated: the agent declined rather than answering ungrounded. "
    ...
    citations: list[Citation] = Field(
        ...
        description=(
            "Required when status is answered. An answer with no citation is escalated instead, "
            "which is control.ai.rag.citation_required doing its job."
        ),
    )
