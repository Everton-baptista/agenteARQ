...
from app import mcp_server  # noqa: E402  (after the helper, keeping the file readable top-down)


def _specs() -> dict[str, dict]:
    return mcp_server.load_tools(mcp_server.load_manifest())


def test_descriptions_hash_is_stable_and_trips_on_change() -> None:
    """The rug-pull tripwire, from the serving side. A description that changes must change
    the hash a consumer pinned, or the pin is decoration."""
    specs = _specs()
    assert mcp_server.descriptions_hash(specs) == mcp_server.descriptions_hash(specs)
    edited = {name: dict(spec) for name, spec in specs.items()}
    first = next(iter(edited))
    edited[first]["description_for_model"] += " one harmless-looking clause"
    assert mcp_server.descriptions_hash(edited) != mcp_server.descriptions_hash(specs)


def _over_stdio(calls):
    """Run a real MCP session against the server as a subprocess and return the payloads.

    A unit test against the handler alone would prove the handler; this proves the protocol —
    that what a client actually receives over stdio is what the manifest declared.
    """
    from mcp import ClientSession, StdioServerParameters
    from mcp.client.stdio import stdio_client

    async def run() -> tuple[set[str], list[dict]]:
        params = StdioServerParameters(
            command=sys.executable,
            args=["-m", "app.mcp_server"],
            env={**os.environ, "AGENT_TENANT": "acme", "AGENT_ROLE": "support_agent"},
        )
        async with stdio_client(params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                return advertised, payloads

    return asyncio.run(run())


def test_over_stdio_the_server_advertises_exactly_the_manifest() -> None:
    """Every declared tool is on the wire, and nothing undeclared is. The schema the client
    validates against is the one a human reviewed in the .tool.yaml."""
    advertised, payloads = _over_stdio([("search_orders", {"order_reference": "BR-77120"})])
    assert advertised == set(_specs())
    assert "error" not in payloads[0], payloads[0]


def test_over_stdio_irreversible_pauses_and_undeclared_is_refused() -> None:
    """An MCP call is a transport, not an approver: the irreversible tool parks instead of
    executing, and a name the manifest never declared gets the refusal, not a guess."""
    _, payloads = _over_stdio(
        [
            ("notify_customer", {"conversation_id": "conv-1", "body": "your order shipped"}),
            ("delete_everything", {}),
        ]
    )
    paused, refused = payloads
    assert paused["status"] == "awaiting_approval"
    assert paused["on_timeout"] == "deny"
    assert paused["effect"] == "communication"
    assert refused == {"error": "no declared tool 'delete_everything'", "retryable": False}
