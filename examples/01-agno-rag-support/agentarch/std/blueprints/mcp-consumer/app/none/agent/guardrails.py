...
def output_guardrail(text: str) -> tuple[bool, str]:
    ...
    No citation requirement: this agent answers from tools on servers it did not write, and there is
    no passage id to check against. A citation rule here would be a check with nothing to check —
    which is worse than no rule, because it reads as assurance.

    What is checked instead is exfiltration. A server-authored tool description can ask the model to
    include a credential or a URL in its reply, so the reply is inspected before it leaves.
    """
