...
```bash
kubectl create secret generic rag-support --from-literal=ANTHROPIC_API_KEY="$YOUR_KEY"
...
| `agent.guardrail.decisions{point="input",decision="block"}` | a sudden rise is either an attack or a miscalibrated guardrail refusing real users, and both need to be seen the same day |
