...
AGENTS_DIR = pathlib.Path("agentarch/project/agents")

# The entry point. Every request arrives here; specialists are only ever reached by a declared
# handoff, never directly from the transport — which is what keeps the delegation graph reviewable
# in the manifest instead of implicit in the routing table.
ENTRY_AGENT = "request-orchestrator"
...
def load_manifest(agent_id: str = ENTRY_AGENT) -> dict:
    ...
    # Kept so a handoff contract's relative payload_schema path can be resolved against the manifest
    # that declared it, rather than against whatever the process working directory happens to be.
    manifest["_dir"] = agent_dir
    return manifest
